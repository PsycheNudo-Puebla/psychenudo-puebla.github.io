levelLogics['dragon'] = {
    ...
    init: function(levelData) {
        ...
        this.cannonImg.src = 'assets/canon.png';
        ...
        this.gameOver = false;
        this.won = false;
        this.dragon = { x: 650, y: 300, dir: 1, speed: 6, fireTimer: 0, animFrame: 0, health: this.questions.length || 3, state: 'normal', dashTimer: 0 };
        this.cannon = { x: 150, y: 300, w: 100, h: 80, loaded: false, bulletType: null, cooldown: 0 };
        this.bullets = [];
        this.activeBullet = { active: false, x: 0, y: 0 };
        this.dragonFire = { active: false, x: 0, y: 0, vx: 0, vy: 0, speed: 14 };
        this.showText = '';
        this.textTimer = 0;
    ...
    update: function() {
        ...
        if (this.dragon.fireTimer > 35) this.dragon.animFrame = 2; 

        if (this.dragon.fireTimer > 50) {
            ...
            const angle = Math.atan2(targetY - this.dragonFire.y, targetX - this.dragonFire.x);
        ...
        if (this.dragonFire.active) {
            ...
            if (this.dragonFire.x < -50 || this.dragonFire.x > 850 || this.dragonFire.y < -50 || this.dragonFire.y > 650) {
                this.dragonFire.active = false;
            }
            
            // Colisión contra el personaje principal
            if (Math.hypot((player.x + player.w / 2) - this.dragonFire.x, (player.y + player.h / 2) - this.dragonFire.y) < 30) {
                this.gameOver = true;
                this.dragonFire.active = false;
        ...
        if (this.activeBullet.active) {
            ...
            // Colisión contra el dragón (Aumentada a 220 por el nuevo tamaño de 600px)
            if (Math.hypot(this.activeBullet.x - this.dragon.x, this.activeBullet.y - this.dragon.y) < 220) {
    ...
    drawDragon: function() {
        ...
        if (img && img.complete) {
            ...
            ctx.fillStyle = this.palette.red_dragon;
